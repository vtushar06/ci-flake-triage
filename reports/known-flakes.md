...
Generated 2026-09-05. 1904 completed runs (2026-06-01 to 2026-09-05), 543 re-run by hand, 798 flakes - 493 of them in 166 signatures, the rest with no marker or no log.

The two oracles are counted separately on purpose, because the evidence behind them is not equally strong:

- **733 rerun-confirmed** - failed on one attempt and passed on a later attempt of the same run, so the commit never changed and there is a passing twin of that exact job to diff against.
...
## no retrievable log (187 flakes)

| n | signature | window | issue candidate |
|---|---|---|---|
| 187 | `(none)` | 2026-06-17 to 2026-09-04 |  |
