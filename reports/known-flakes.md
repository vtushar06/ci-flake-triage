...
Generated 2026-09-18. 2094 completed runs (2026-06-01 to 2026-09-18), 603 re-run by hand, 876 flakes - 493 of them in 166 signatures, the rest with no marker or no log.

The two oracles are counted separately on purpose, because the evidence behind them is not equally strong:

- **811 rerun-confirmed** - failed on one attempt and passed on a later attempt of the same run, so the commit never changed and there is a passing twin of that exact job to diff against.
...
## no retrievable log (265 flakes)

| n | signature | window | issue candidate |
|---|---|---|---|
| 265 | `(none)` | 2026-06-17 to 2026-09-18 |  |
